...
def chart_series_names(docx_path: Path, chart_xml_name: str) -> list[str]:
    chart_ns = {"c": "http://schemas.openxmlformats.org/drawingml/2006/chart"}
    with ZipFile(docx_path) as zf:
        root = ET.fromstring(zf.read(chart_xml_name))
    names: list[str] = []
    for series in root.findall(".//c:ser", chart_ns):
        value = series.find(".//c:tx//c:v", chart_ns)
        if value is not None and value.text:
            names.append(value.text)
    return names


def chart_xml_by_series(docx_path: Path, required_markers: tuple[str, ...], fallback_index: int) -> str:
    charts = chart_xml_names(docx_path)
    for chart_xml in charts:
        series_names = chart_series_names(docx_path, chart_xml)
        if all(any(marker in name for name in series_names) for marker in required_markers):
            return chart_xml
    return charts[fallback_index] if len(charts) > fallback_index else ""
...
def compact_market_sentence(text: str) -> str:
    match = re.match(r"(.+?国债收益率全期限下行。)", text)
...
def build_variant(variant: str, docx_path: Path, rules: dict) -> dict:
    ...
    market_sentence = compact_market_sentence(paragraphs[market_idx + 1])
    ...
    risk_preference = find_text(paragraphs, "市场风险偏好：")
    try:
        funding = find_text(paragraphs, "银行间市场流动性：")
    except ValueError:
        funding = find_text(paragraphs, "资金面：")
    ...
    fields = {
        ...
        "funding": strip_prefix(strip_prefix(funding, "银行间市场流动性："), "资金面："),
    ...
    yield_chart_xml = chart_xml_by_series(docx_path, ("中债国债到期收益率:1年",), 0)
    fund_chart_xml = chart_xml_by_series(docx_path, ("回购加权利率:1天", "回购加权利率:7天"), 1)
    ...
    return {
        ...
        "assets": {
            ...
            "yield_chart_xml": yield_chart_xml,
            "fund_chart_xml": fund_chart_xml,
